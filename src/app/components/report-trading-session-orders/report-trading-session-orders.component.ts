...
import {
  ...
  DemandOffer,
  TradingSessionDemandOffersField,
  DemandOfferGood,
...
export const REPORT_TRADING_SESSION_ORDERS = 'REPORT_TRADING_SESSION_ORDERS';
export const REPORT_TRADING_SESSION_ORDERS_PAGE_NAME =
  'report-trading-session-orders';
export const REPORT_TRADING_SESSION_ORDERS_KEY = 'reportTradingSessionOrders';
export const REPORT_TRADING_SESSION_ORDERS_HIDE_LOT_ITEMS_KEY: string =
  'reportTradingSessionOrdersHideLotItems';

const REPORT_TRADING_SESSION_ORDERS_LOT_DETAIL_COLUMNS: string[] = [
  'goodName',
  'cnfea',
  'goodDescription',
  'priceWithoutVatOriginal',
  'priceWithoutVatStarting',
  'priceWithoutVatStartingTrading',
  'priceWithoutVatCurrent',
  'goodVolume',
  'goodUnitName',
  'concatedDeliveryConditionAdd',
];

export const REPORT_TRADING_SESSION_ORDERS_FILTERS = {
...
@Component({
  selector: 'app-report-trading-session-orders',
  templateUrl: './report-trading-session-orders.component.html',
  styleUrls: ['./report-trading-session-orders.component.scss'],
})
export class ReportTradingSessionOrdersComponent implements OnInit {
  ...
  public readonly REPORT_TRADING_SESSION_ORDERS_FILTERS =
    REPORT_TRADING_SESSION_ORDERS_FILTERS;
  public readonly REPORT_TRADING_SESSION_ORDERS_PAGE_NAME =
    REPORT_TRADING_SESSION_ORDERS_PAGE_NAME;
  public readonly REPORT_TRADING_SESSION_ORDERS_KEY =
    REPORT_TRADING_SESSION_ORDERS_KEY;

  public user: User;
  public listDemandOffer: DemandOffer[];
  public dynamicFields: TradingSessionDemandOffersField[];
  ...
  public ngOnInit(): void {
    const cache: PageCache = this.sessionStorageService.getItemFromSessionStorage<PageCache>(
    ...
    this.checkForTradingSessionDemandOffers();

    this.user = this.localStorageService.getItemFromLocalStorage('user');

    const R_TRADING_SESSION_ORDER = getTranslateResultByCurrentLang(
      this.translate.store.currentLang,
      'report-trading-session-orders.reportTradingSessionOrders'
    );
    const faviconUrl = 'assets/img/icons/offer-managment.svg';
    this.pageMeta.setPageMeta(null, R_TRADING_SESSION_ORDER, faviconUrl);
  }
  ...
  private restoreHideLotItemsState(): void {
    this.isHiddenLot =
      this.localStorageService.getItemFromLocalStorage<boolean>(
        REPORT_TRADING_SESSION_ORDERS_HIDE_LOT_ITEMS_KEY
      ) ?? false;
  }
  ...
  private applyLotColumnsVisibility(): void {
    ...
    REPORT_TRADING_SESSION_ORDERS_LOT_DETAIL_COLUMNS.forEach((field: string): void => {
      instance.columnOption(field, 'visible', visible);
    });
    this.dynamicFields?.forEach((field: TradingSessionDemandOffersField): void => {
  ...
  private checkForTradingSessionDemandOffers(): void {
    ...
    if (
      ...
      this.getTradingSessionDemandOffers(token, filters);
    }
  }
  ...
  public getDataSourceDynFilter(idFieild: ID_INTERFACE_FIELD): DemandOffer[] {
    let results = [];

    this.listDemandOffer.forEach((item) => {
  ...
  public priceWithoutVatOriginalFilter = <T extends FilterData>(
    data: T
  ): void => {
    return this.getUniqueResult(data, 'priceWithoutVatOriginal');
  };

  public priceWithoutVatStartingFilter = <T extends FilterData>(
    data: T
  ): void => {
    return this.getUniqueResult(data, 'priceWithoutVatStarting');
  };

  public priceWithoutVatStartingTradingFilter = <T extends FilterData>(
    data: T
  ): void => {
    return this.getUniqueResult(data, 'priceWithoutVatStartingTrading');
  };

  public priceWithoutVatCurrentFilter = <T extends FilterData>(
    data: T
  ): void => {
    return this.getUniqueResult(data, 'priceWithoutVatCurrent');
  };
  ...
  public lotSummaryVolumeFilter = <T extends FilterData>(data: T): void => {
  ...
  public goodConcatedAddCondFilter = <T extends FilterData>(data: T): void => {
    return this.getUniqueResult(data, 'concatedDeliveryConditionAdd');
  };
  ...
  private getUniqueResult(
    data: FilterData,
    field: keyof DemandOfferGood
  ): void {
    let hasNonEmptyValues: boolean = false;

    data.dataSource.postProcess = () => {
      const results = this.listDemandOffer.reduce((acc, item) => {
        const goodsItems = item.goods
          .map((el) => {
  ...
  private getTradingSessionDemandOffers(
    sessionKey: string,
    body: TradingSessionDemandOffersBody
  ) {
    this.reportService
      .getTradingSessionDemandOffers(sessionKey, body)
      .pipe(
        tap((data) => {
          this.listDemandOffer = data.demandOffers;
          this.dynamicFields = data.fields;

          if (this.listDemandOffer) {
            this.mapListDemandOffers();
          }
        }),
        catchError((error: Error) => {
          console.error(
            'Ошибка при загрузке заявок на торговую сессию:',
            error
          );
  ...
  private mapListDemandOffers(): void {
    this.listDemandOffer = this.listDemandOffer.map((item) => {
      ...
      const priceWithoutVatOriginal = item.goods.map(
        (x) => x.priceWithoutVatOriginal
      );
      const priceWithoutVatStarting = item.goods.map(
        (x) => x.priceWithoutVatStarting
      );
      const priceWithoutVatStartingTrading = item.goods.map(
        (x) => x.priceWithoutVatStartingTrading
      );
      const priceWithoutVatCurrent = item.goods.map((x) => x.priceWithoutVatCurrent);
      const mainName = getMainName(item.goods);
      const concatedDeliveryConditionAdd = item.goods.map(
        (x) => x.concatedDeliveryConditionAdd
      );
      ...
      if (this.dynamicFields) {
        dynamicFieldsValues = this.dynamicFields.reduce((acc, dyn) => {
          ...
          return acc;
        }, {} as Record<string, ID_INTERFACE_FIELD[]>);
      }
  ...
  private generateFilters(): TradingSessionDemandOffersBody {
    const requestObject: TradingSessionDemandOffersBody = {
  ...
  public exportGrid(e: ExportingEvent): void {
    ...
    const reportOrders: string = getTranslateResultByCurrentLang(
      this.translate.store.currentLang,
      'header.top_line.menu.report-trading-session-orders'
    );
    ...
    const filters: TradingSessionDemandOffersBody = this.generateFilters();
    ...
    const fileName: string = `${reportOrders}, ${sectionName}, ${dynamicPart}`;

    this.exportService.onExportingReports(
      e,
      fileName,
      FileTypes.REPORT_ORDERS,
