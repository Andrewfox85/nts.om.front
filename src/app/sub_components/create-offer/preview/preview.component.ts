...
export class PreviewComponent implements OnInit {
  ...
  // @Input() generalInfo; //общая информация
  ...
  public getDisplayValue(field: FieldData | any): string {
    const controlType: string = field?.interfaceField?.controlFieldType;

    if (controlType === 'dxSelectBox') {
      return this.valueSelectBox(field);
    }

    if (controlType === 'dxCheckBox') {
      return field.selectedValues?.toString() === 'true'
    ...
    if (controlType === 'dxNumberBox') {
      if (!field.selectedValues) {
        return '';
      }
      const precision: number = field.interfaceField.fieldPrecision || 0;
      return this.formatNumber(field.selectedValues, precision);
    }

    if (field?.costNoVAT || field?.amountVAT || field?.amountVAT === 0 || field?.costVAT) {
      return this.formatNumber(field.selectedValues, this.currencyPrecision);
    }
  ...
  private formatNumber(value: number, precision: number): string {
    ...
    return value.toLocaleString('ru', { minimumFractionDigits: precision });
  }
